    ...
    if (profile) {
        ...
        if (badgesContainer) {
            ...
            if (profile?.availabilityNote) {
                ...
                noteContainer.style.marginTop = '12px';

                const noteLabel = document.createElement('div');
                noteLabel.innerHTML = '<span class="profile-label">📝 수업 불가능 시간 메모</span>';
                noteLabel.style.marginBottom = '6px';

                const noteContent = document.createElement('div');
                noteContent.className = 'profile-value';
                noteContent.style.whiteSpace = 'pre-wrap';
                noteContent.style.padding = '8px 12px';
                noteContent.style.backgroundColor = '#f8f9fa';
                noteContent.style.borderRadius = '6px';
                noteContent.textContent = profile.availabilityNote;

                noteContainer.appendChild(noteLabel);
                noteContainer.appendChild(noteContent);
                badgesContainer.appendChild(noteContainer);
            }
        }
